 ...
 *
 * <p>
 * Generates brown noise (aka brownian noise or red noise) by integrating white
 * noise, which is generated using {@link java.util.Random}. A simple high-pass
 * filter is used to reduce DC drift and clipping.
 *
 * <p>
...
public class BrownNoiseGenerator {
	 ...
	 *            Maximum output value.
	 * @param slope
	 *            Maximum slope of the output signal. This parameter defines the
	 *            maximum difference between two consecutive output values. It
	 *            controls the amplitude of the output signal spectrum. A
	 *            reasonable value is <code>(maxValue - minValue) / 20</code>.
	 * @param hpFilter
	 *            High pass filter factor. A simple first-order high pass filter
	 *            is used to reduce DC drift of the output signal and avoid
	 *            frequent clipping. A reasonable value is 0.02. If this
	 *            parameter is zero, the high pass filter is disabled.
	 */
	public BrownNoiseGenerator(double minValue, double maxValue, double slope, double hpFilter) {
		...
		// slope must be less than valueRange/2 because of the way we handle
		// clipping.
	...
	public double getNext() {
		...
		// We use Random.nextFloat() because it's faster than nextDouble() and
		// we
